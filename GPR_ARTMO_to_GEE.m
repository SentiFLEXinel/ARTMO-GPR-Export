...
overall_model_gee_f = fullfile(dir_out,'ARTMO_model_GEE.txt');
...
if fid
    ...
    fprintf(fid, "var veg_index = '%s' ;\n", vi);
    [pathstr, modelFileName, ext] = fileparts(model_mat_f);
    fprintf(fid, "var model = '%s' ;\n", modelFileName);
    fprintf(fid, ['var scaleFactor' suffix ' = 10000;\n']);
    fprintf(fid, '\n');
end
fclose(fid);
...
disp('ARTMO model successfully exported (JavaScript for GEE editor)');
