function [out] = GPR_ARTMO_to_pyeogpr(model_mat_f, suffix, dir_out)


prec_digits = 15;

% Get the file parts (directory and file name without extension)
[mat_dir,mat_f,~] = fileparts(model_mat_f);

% Set default suffix if not provided
if ~exist('suffix','var'); suffix = ''; end

% Set the output directory to the same as the input if not provided
if ~exist('dir_out','var')
    dir_out = fullfile(mat_dir,mat_f);
    if ~exist(dir_out,'dir');mkdir(mat_dir,mat_f);end
end

% Load the model data from the .mat file
...
% Convert data into strings for Python file
Xtr_str     = matrix2str(Xtr,        ['X_train' suffix ' = np.array(['], '['  ,''  ,','    , ']'   ,',' ,prec_digits ,']);' );
alpha_str   = matrix2str(alpha',     ['alpha_coefficients' suffix ' = np.array([['], ''   ,''  ,','    , ''    ,''  ,prec_digits ,']]);' );
mx_str      = matrix2str(mx,         ['mx' suffix '       =  np.array([['], ''   ,''  ,','    , ''    ,''  ,prec_digits ,']]);' );
sx_str      = matrix2str(sx,         ['sx' suffix '       =  np.array([['], ''   ,''  ,','    , ''    ,''  ,prec_digits ,']]);' );
mean_str    = matrix2str(mean,       ['mean_model' suffix ' = '], '', '', ',', '', '', prec_digits, ';' );
inv_l2_str  = matrix2str(invell2_v', ['hyp_ell' suffix '  = np.array(['], ''   ,''  ,','    , ''    ,''  ,prec_digits ,']);' );
sf2_str     = matrix2str(sf2,        ['hyp_sig' suffix '  ='], '', '', ',', '', '', prec_digits, ';'   );
s2_str      = matrix2str(s2,         ['hyp_sign' suffix ' = np.array(['], ''   ,''  ,''     , ''    ,''  ,prec_digits ,']);'   );
XTDX_str    = matrix2str(XTDX,       ['XDX_pre_calc' suffix ' =  np.array(['], '[', '[', '],'  , ']'  ,','  ,prec_digits ,']);' );
Linv_str    = matrix2str(Linv_mat,   ['Linv_pre_calc' suffix ' =  np.array(['], '[', '', ',', ']'  ,','  ,prec_digits ,']);' );
s2unc_str   = matrix2str(s2unc,      ['hyp_sig_unc' suffix ' = '], '', '', ',', '', '', prec_digits, ';' );     

% Path to the overall_model_py.txt file
overall_model = fullfile(dir_out, 'ARTMO_model_pyeogpr.py');

% Open the file for writing
fid = fopen(overall_model, 'wt');
if fid == -1
    error('Could not open file for writing: %s', overall_model);
end
...
disp('ARTMO model successfully exported (Python for PyEOGPR package)');

% Helper function to split strings for writing to file
...
% Helper function to convert matrix to a formatted string
